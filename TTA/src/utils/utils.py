...
def safe_list_to(data, device):
    ...
    if isinstance(data, torch.Tensor):
        ...
        # Optional dependency: torch_geometric
        try:
...
def _read_gene(omics_dir, split):
    ...
    if os.path.isfile(split_path):
        ...
        if 'sample' not in df.columns:
            raise ValueError("Gene data missing 'sample' column!")
        
        def extract_case_id(sample_id):
            if pd.isna(sample_id):
                return sample_id
            # TCGA-AR-A5QQ-01 -> TCGA-AR-A5QQ
        ...
        df['case_id'] = df['sample'].apply(extract_case_id)
        
        first_col = df.columns[0]
        if 'Unnamed: 0' in df.columns:
            df = df.drop('Unnamed: 0', axis=1)
        elif first_col == '' or first_col.isdigit() or str(first_col).startswith('Unnamed'):
            df = df.drop(first_col, axis=1)
        
        print(f"Gene data loaded: {len(df)} samples")
        print(f"Sample case_id examples: {df['case_id'].head().tolist()}")
...
class EarlyStopping:
    ...
    def __call__(self, epoch, score, save_ckpt_fn, save_ckpt_kwargs):
        # Completely skip best-check and saving during warm-up epochs.
        # This avoids comparing against early noisy minima and prevents any
        # "Saving model ..." messages before min_stop_epoch.
        if (epoch + 1) <= self.min_stop_epoch:
            print(
                f'EarlyStopping warm-up: epoch {epoch+1} <= min_stop_epoch {self.min_stop_epoch}, skip best-check and patience')
